...
(package io { .use ( std ) }
  ...
  (defmacro with-open-dir (name+arg* . body)
    (let ((name (car name+arg*))
          (arg* (cdr name+arg*)))
      (with-gensyms (result)
        `(let ((~name (io:open-dir @arg*)))
           (let ((~result (progn @body)))
             (io:close-dir ~name)
             ~result)))))

  (defun fold-dir (f init path)
    (with-open-dir (dir path)
      (rlet rec ((accum init))
        (let ((dirent (read-dir dir)))
          (if dirent
              (rec (f accum dirent))
            accum)))))

  (export with-open-file write
          with-open-dir  fold-dir))
