...
rtl_Word rtl_repl_load(rtl_Machine *M, rtl_Word const *args, size_t argsLen)
{
  ...
  scratchFnID = rtl_newFuncID(M->codeBase, rtl_intern("repl", "scratch"));

  while (!feof(file)) {
    w = rtl_read(C, file);

    rtl_compile(C, wrapper.ns, scratchFnID, w);
    ...
    rtl_disasmFn(C->M->codeBase, rtl_function(scratchFnID));
